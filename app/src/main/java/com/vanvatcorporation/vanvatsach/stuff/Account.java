...
public class Account implements Serializable {
    /**
     * The uuid is get from login process, in which it save to the storage of the
     * device. If I can read the uuid then it retrieve only the username of the
     * login process.
     * Hacker modifies the code to retrieve password instead? Change the server-side
     * code to only send username
     */
    public String uuid;
    ...
    public boolean isValid(Context context) {
        return valid || (AuthRepository.getInstance(context).getCurrentUser() != null);
    }
    ...
    public static Account setupAccount(Context context) {
        ...
        if (accountInformation.accountUUID == null || accountInformation.accountUsername == null
                || accountInformation.accountPassword == null || accountInformation.accountEmail == null)
            return null;
        if (accountInformation.accountUUID.isEmpty() || accountInformation.accountUsername.isEmpty()
        // || accountInformation.accountPassword.isEmpty()
                || accountInformation.accountEmail.isEmpty())
            return null;

        Account account = Account.login(context, accountInformation);
    ...
    public static Account setupAccount(Context context, AccountInformation accountInformation) {
        ...
        account.age = DateHelper
                .getDeltaOfYearTimeWithCurrentYearTime(DateHelper.getYearOfTimestamp(account.legalBirthday));
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Account.AccountInformation.AccountData dataClone = new Gson().fromJson(new Gson().toJson(data),
                            AccountInformation.AccountData.class);
                    dataClone.boughtAssets = null;

                    JsonObject accountStatistics = new JsonObject();

                    accountStatistics.add("accountStatistics",
                            JsonParser.parseString((new Gson().toJson(dataClone.accountStatistics))));

                    JsonObject object = new JsonObject();
                    object.add("accountUsername",
                            new JsonPrimitive(DynamicConstants.temporaryAccountInformation.accountUsername));
                    object.add("accountPassword",
                            new JsonPrimitive(DynamicConstants.temporaryAccountInformation.accountPassword));
